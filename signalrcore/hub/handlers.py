...
class StreamHandler(object):
    def __init__(self, event, invocation_id):
        ...
        self.next_callback =\
            lambda _: logging.warning("next stream handler fired, no callback configured")
        self.complete_callback =\
            lambda _: logging.warning("next complete handler fired, no callback configured")
        self.error_callback =\
            lambda _: logging.warning("next error handler fired, no callback configured")
    
    def subscribe(self, subscribe_callbacks):
        error = " subscribe object must be a dict like {0}"\
            .format({
                "next": None,
                "complete": None,
                "error": None
